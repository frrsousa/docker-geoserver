...
# Fazer download direto do GeoServer WAR (leve)
RUN curl -L -o $CATALINA_HOME/webapps/geoserver.war https://build.geoserver.org/geoserver/${GEOSERVER_VERSION}/geoserver-${GEOSERVER_VERSION}-war.zip && \
    unzip -o $CATALINA_HOME/webapps/geoserver.war -d $CATALINA_HOME/webapps/geoserver || true

# Desativar GeoWebCache (reduz consumo de RAM)
