...
# Instalar wget e unzip para descarregar o WAR
...
# Copiar estrutura mínima data_dir (logs, styles, workspaces, security, etc.)
COPY data_dir ${GEOSERVER_DATA_DIR}

# Garantir permissões corretas para evitar erro 400
RUN chmod -R 777 ${GEOSERVER_DATA_DIR} && \
    chmod -R 755 ${GEOSERVER_HOME} && \
    chown -R root:root ${GEOSERVER_HOME}

# Definir variáveis de ambiente do GeoServer
